...
function renderCollapsed(prose: HTMLElement, id: string, title: string, snippet: string) {
...
function renderExpanded(prose: HTMLElement, id: string, title: string, fullHTML: string) {
...
function initTruncation(prose: HTMLElement) {
    ...
    prose.addEventListener('click', (e) => {
        const btn = (e.target as HTMLElement).closest<HTMLElement>('[data-event-action]');
        if (!btn) return;
        if (btn.dataset.eventAction === 'expand') renderExpanded(prose, id, title, fullHTML);
        else if (btn.dataset.eventAction === 'collapse') renderCollapsed(prose, id, title, snippet);
    });
}

document.querySelectorAll<HTMLElement>('[data-event-prose]').forEach(initTruncation);
